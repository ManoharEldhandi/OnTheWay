...
| --- | --- | --- | --- |
| `ontheway.eta.scheduler-enabled` | `ETA_SCHEDULER_ENABLED` | `true` | Enables automatic `ACCEPTED` → `PREPARING` progression. Disable for deterministic test runs. |
...
While the customer is **en route** (order status `ACCEPTED` or `PREPARING`), their app streams
