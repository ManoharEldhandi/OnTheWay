...
- [ ] **T1.1** Add shop **status** (`PENDING`/`APPROVED`/`REJECTED`/`SUSPENDED`) + reason; migration.
- [ ] **T1.2** Allow **one owner → many shops** (owner relationship; new shops start `PENDING`).
- [ ] **T1.3** Discovery returns **only `APPROVED`, non-suspended** shops.

### Merchant self-service
- [ ] **T1.4** `POST /api/merchant/shops` to **apply** for a shop (status `PENDING`).
- [ ] **T1.5** `GET /api/merchant/shops` to list the merchant's own shops with status.
- [ ] **T1.6** Menu self-service on own shops: add / edit / **change price** / **out-of-stock toggle** / delete.
- [ ] **T1.7** Merchant order queue: list and advance orders for own shops only.

### Admin console
- [ ] **T1.8** `GET /api/admin/applications` pending queue; **approve** / **reject (reason)**.
- [ ] **T1.9** `POST /api/admin/shops/{id}/suspend|reactivate`, `DELETE /api/admin/shops/{id}`.
- [ ] **T1.10** `GET /api/admin/metrics` platform overview (counts, orders, revenue, pending, status mix).
- [ ] **T1.11** `GET /api/admin/shops` directory with status filter.
...
- [ ] **T2.1** Expand category taxonomy (10+ verticals).
...
- [ ] **T4.3** Seed **100+ shops across many verticals** for a credible demo.
